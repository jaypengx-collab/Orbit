...
- 已配對的裝置可以另外勾選「不同步樣式顏色」——之後這台裝置永遠保留自己目前的主色／次色／個人樣式，其他人改配色不會影響它，但課表內容照常同步。
...
**同步怎麼運作**：按下「儲存」的當下就會立刻上傳，不用等下一輪——存檔即同步，不是存檔後還要等幾秒才輪到。除此之外每台裝置也會定期檢查遠端有沒有更新版本，有就套用（用 Firestore `updateTime` 判斷新舊）：一開始每 8 秒查一次，如果連續幾次都沒東西可拉，間隔會逐漸拉長（最長到 60 秒）——兩台裝置放著不動時沒必要一直問，讀取次數自然降下來；一旦真的抓到變化，或分頁重新回到前景／重新整理，馬上跳回每 8 秒的頻率，不會讓人等一整輪拉長後的間隔才看到更新。編輯器有未儲存變更時同步暫停讀寫；分頁在背景時不發請求。

**衝突處理**：**最後寫入者獲勝（last-write-wins）**，沒有欄位層級合併，沒有版本分支。

- 一台編輯、其他裝置只顯示（最常見）：管理者按下儲存後立刻上傳，其他裝置通常幾秒內（視當下輪詢間隔）就會拉到，沒有衝突。
...
- 沒有帳號系統，沒有多人協作；跨裝置同步是輪詢式（存檔會立即上傳，但接收端仍是每 8–60 秒回頭問一次，視最近有沒有變化自動調整），不是即時推播。
