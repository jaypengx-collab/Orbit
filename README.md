...
編輯器裡的「跨裝置同步」是選用功能，會把課表自動同步到多台裝置，不用每次都手動匯出/匯入。Orbit AI 本身沒有伺服器，所以同步用的是一個共用的 Firebase 專案，資料透過 Firestore 的 REST API 直接用 `fetch` 存取（沒有引入 Firebase SDK，維持專案原本「無額外依賴」的作法）。裝置數量沒有上限——任何裝置只要拿到同一組配對代碼，就會加入同一份共享文件。

**一般使用者**：部署站台已經內建了同步伺服器（見下方部署者設定），編輯器裡的「跨裝置同步」只會要求輸入一組配對代碼，其他什麼都不用填。在裝置 A 按「建立新同步」拿到代碼，在裝置 B 貼上同一組代碼按「加入同步」即可。
