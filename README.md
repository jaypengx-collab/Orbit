...
- 僅接收裝置打開編輯器時，除了「同步 / 匯入匯出」面板本身（查看代碼、解除同步），其他頁面整頁鎖住變暗；同面板裡的 AI 匯入、手動「匯入」按鈕也個別鎖住（手動「匯出」不受影響）；儲存功能直接拒絕。頂部工具列的「樣式工具」按鈕也一併鎖住——還在接收同步樣式的僅接收裝置改了也沒用，下次同步就會被蓋掉；勾選「不同步樣式顏色」後才會解鎖。
