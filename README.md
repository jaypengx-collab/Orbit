...
跨裝置同步的配對資訊也存在獨立的鍵：`orbitSyncProjectId`（Firebase 專案 ID）、`orbitSyncCode`（配對代碼）、`orbitSyncLastUpdateTime`（最後一次同步到的 Firestore `updateTime`，用來判斷遠端是否有更新）。沒有設定同步的話，這幾把鍵不會存在。
