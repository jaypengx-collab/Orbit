...
- 「解除同步」只是這台裝置自己忘記配對代碼，伺服器上的共用課表跟其他已配對的裝置完全不受影響，代碼也還能拿去重新加入。管理者另外看得到「整個刪除同步」——這個會透過 Worker 把伺服器上那份共用課表整個刪掉，配對代碼直接失效：所有用這組代碼加入的裝置（不只按下按鈕的這台）之後同步時都會發現代碼已不存在，各自變回自己最後一次收到的本機課表，且無法復原（詳見下方安全性一節的 `DELETE /sync`）。僅接收裝置看不到這顆按鈕——即使繞過介面直接呼叫，Worker 那端也一樣會執行刪除，這層限制單純是介面引導，不是真正的權限系統。
