...
**一般使用者**：只需要輸入一組配對代碼。裝置 A 按「建立新同步」拿代碼，裝置 B 貼上同一組代碼按「加入同步」。配對/解除配對後畫面立刻切換，不用重新整理。加入前一定會先跳出警告：如果代碼下已有課表，加入會立刻取代這台裝置目前的課表且無法復原（建立同步的裝置不受影響）。代碼打錯或對方根本沒建立過的話，加入會直接失敗並提示找不到代碼，不會誤報成功、也不會把這台裝置留在「已配對但其實沒東西可收」的狀態。
