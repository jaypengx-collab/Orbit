...
**管理者／僅接收身份**：配對時分「管理者」與「僅接收」兩種身份——建立同步的裝置一定是管理者；用代碼加入的裝置預設是僅接收，只會自動收到管理者發布的更新，編輯器裡除了「同步 / 匯入匯出」面板本身（用來查看代碼、解除同步），其他所有頁面都會整頁鎖住、變暗且無法點擊，儲存功能也會直接拒絕，避免不小心改到共用課表。想讓一台加入的裝置也能編輯，加入時勾選「以管理者身份加入」即可——管理者可以不只一個，只是預設每個新加入的裝置都先當觀眾。僅接收裝置想自己改課表的話，按「解除同步」即可，本機課表不受影響，也可以之後再重新加入。這一層鎖定純粹是介面上的引導，不是真正的權限系統（跟同步本身一樣，見〈限制〉）——真的要修改，直接在瀏覽器主控台呼叫也繞得過去，只是不小心點到的話不會誤觸。
