...
編輯器裡的「跨裝置同步」是選用功能，會把課表自動同步到多台裝置，不用每次都手動匯出/匯入。Orbit AI 本身沒有伺服器，所以同步用的是使用者自己的免費 Firebase 專案，資料透過 Firestore 的 REST API 直接用 `fetch` 存取（沒有引入 Firebase SDK，維持專案原本「無額外依賴」的作法）：
...
2. 在 Firestore 的「規則」分頁貼上：
   ...
   ```
   這條規則完全公開（任何知道專案 ID 與配對代碼的人都能讀寫該筆資料），沒有做額外的存取控制或驗證——配對代碼本身（8 碼、排除易混淆字元）是唯一的門檻。在意資料是否可能被外部存取的使用者，不建議開啟這項功能。
3. 回到 Orbit AI 的編輯器 →「跨裝置同步」，貼上 Firebase 專案 ID，按「建立新同步」——會產生一組配對代碼並立刻上傳目前課表。
4. 在另一台裝置的同一個區塊貼上相同的專案 ID 與代碼，按「加入同步」。
...
一般使用（建課表、看主畫面、備份匯出匯入）完全在瀏覽器本機進行，沒有任何資料離開你的裝置。有兩個主動選用的例外：使用 AI 辨識功能時，你選的那張照片會被送到 Google 的 Gemini API 做分析；開啟跨裝置同步時，課表資料會存到你自訂的 Firebase 專案（見〈跨裝置同步〉），而且該功能預設用的是完全公開讀寫的 Firestore 規則，不做任何身分驗證。這兩者都必須使用者自己主動設定才會啟用，不用的話完全不需要申請或輸入任何金鑰、專案 ID。Gemini API Key 只存在瀏覽器本機，不會被寫進原始碼，也不會跟著課表備份一起匯出。
...
- 跨裝置同步需要使用者自己建立 Firebase 專案，且預設規則完全公開讀寫，不做任何身分驗證或加密——不是為了保護隱私設計的，在意這點的話不建議開啟。
- AI 辨識的準確度取決於照片清晰度與課表版型，不是每次都能完美讀出所有欄位，匯入前務必看過預覽再確認。
- AI 功能需要網路連線與自備的 API Key，離開這兩個條件就無法使用（但其他功能完全不受影響）。
- 自動化測試涵蓋課表計算、資料驗證、備份格式與同步模組的請求/套用邏輯，但編輯器 UI 流程、樣式面板、AI 匯入流程目前仍主要靠時間模擬手動驗證。
