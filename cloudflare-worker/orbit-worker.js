...
A common pairing: one file is a weekly timetable grid where some cells show a generic, category-level label instead of one specific subject — a name for an elective block, a self-study/flexible period, a "choose your own" track, or similar, rather than the name of an actual course. Another file is a course-registration record: a list or table where each row names one specific course a particular student is actually enrolled in, generally alongside which day and period(s) that course meets. That per-row day/period information may be written out plainly (a day name and a period number in their own columns or sentences) or packed together compactly in whatever shorthand that document's author used (for instance a day symbol immediately followed by one or more period digits, each digit marking one period that row's course occupies that day) — read it the same way you would read any other compact notation in that same document, using whatever day-naming and period-numbering convention the rest of the document already establishes. When you see this pairing:
