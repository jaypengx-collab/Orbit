...
function buildSchedule() {
  ...
  [0, 1, 2, 3, 4, 5, 6].forEach(day => {
    runtimeSchedule[day] = (applicationData.weeklySchedule[day] || [])
      .map((key, i) => {
        if (!key) return null;
        const m = applicationData.teacherDB[key] || ['未知', '未知'];
        ...
        return {
          ...
          loc: applicationData.locationDB[key] || ''
        };
      })
      .filter(Boolean);
  });
...
function renderNavBar() {
  ...
  const hasWE =
    (runtimeSchedule[0] && runtimeSchedule[0].length > 0) ||
    (runtimeSchedule[6] && runtimeSchedule[6].length > 0);
  if (hasWE) {
    if (runtimeSchedule[6] && runtimeSchedule[6].length > 0) days.push(6);
    if (runtimeSchedule[0] && runtimeSchedule[0].length > 0) days.push(0);
  }
  const labels = {
    0: '週日',
    1: '週一',
...
function renderStylePanel() {
  stylePanelDraft = {
    ...applicationData,
    styleSlots: normalizeStyleSlots(applicationData.styleSlots)
  };
...
function getStyleDraftFromControls() {
  ...
  return {
    ...stylePanelDraft,
    proAccent: normalizeProAccent(primary?.value || stylePanelDraft.proAccent),
    proSecondary: normalizeProSecondary(secondary?.value || stylePanelDraft.proSecondary),
    proTertiary: normalizeProSecondary(secondary?.value || stylePanelDraft.proSecondary)
  };
}
...
function saveStyleSlotDraft(index) {
  ...
  slots[index] = {
    ...
    primary: stylePanelDraft.proAccent,
    secondary: stylePanelDraft.proSecondary
  };
...
function loadStyleSlot(index) {
  const slot = normalizeStyleSlots(stylePanelDraft?.styleSlots || applicationData.styleSlots)[
    index
  ];
...
function update() {
  ...
  const activeBreak =
    curIdx === -1
      ? (applicationData.breakTimes || []).find(
          item =>
            item.name &&
            item.start &&
            item.end &&
...
function collectEditorFormState() {
  ...
  const proSecondary = normalizeProSecondary(
    applicationData.proSecondary || derivedProColors.secondary
  );
...
function applyEditorSettingsData(next, { closeAfter = false, statusMessage = '' } = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(applicationData, 'geminiApiKey'))
    setStoredGeminiApiKey(applicationData.geminiApiKey);
...
function getEditorBellPeriodCount() {
  ...
  return n > 0 ? n : (applicationData.bellTimes || []).length;
}
...
function getEditorUnsavedDiff() {
  try {
    ...
    return describeSettingsDiff(
      editorBaselineData || normalizeSettingsData(applicationData),
...
function renderEditorTeachers() {
  ...
  (applicationData.teacherOrder || Object.keys(applicationData.teacherDB))
    .filter(key => applicationData.teacherDB[key])
    .forEach(key => {
...
function getClassColor(key) {
  ...
  const activeStyle =
    stylePanelDraft && draftPanel?.classList.contains('style-draft-dirty')
      ? stylePanelDraft
      : applicationData;
